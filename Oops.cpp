...
public:
    ...
    ~Order() {
        for (auto item : items) {
            // We do not delete here because the MenuItem objects are owned by Restaurant
            // Deleting them here would lead to double deletion issues.
        }
    }
...
private:
    ...
    // Static variables to track total orders and revenue
...
public:
    ...
    void takeOrder() {
        ...
        if (order->getTotal() > 0) {
            ...
            totalRevenue += order->getTotal();  // Add to total revenue
